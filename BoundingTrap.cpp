...
BoundingTrap<PointType, OrderType>::edge_destruction(Cut<PointType, OrderType>* cut) {
    ...
    //         if (Segment<PointType, OrderType>::slope_comparison(*top->get_segment(), *cut->get_segment()) != 0) {
...
template <class PointType, class OrderType>
bool BoundingTrap<PointType, OrderType>::through_common_corner(BoundingTrap<PointType, OrderType>& other, Segment<PointType, OrderType>* seg) {

    if (bottom == other.get_top()) {

        if (Segment<PointType, OrderType>::slope_comparison(*bottom->get_segment(), *seg) == 0)
            assert(false);
    ...
    } else if (top == other.get_bottom()) {

        if (Segment<PointType, OrderType>::slope_comparison(*top->get_segment(), *seg) == 0)
             assert(false);
...
bool BoundingTrap<PointType, OrderType>::contains_defining_point(Cut<PointType, OrderType>* cut) {
    assert(cut->get_cut_type() != EDGE);
    return cut->defining_point_cut_comparison(*top) < 1 &&
    cut->defining_point_cut_comparison(*bottom) > -1 &&
    cut->defining_point_cut_comparison(*get_left()) > -1 &&
    cut->defining_point_cut_comparison(*get_right()) < 1;
}
